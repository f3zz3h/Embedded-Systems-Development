...
class RTSPClient(basic.LineReceiver):
    ...
    url = None # Url to send to gstreamer
    ...
    def dataReceived(self, data):
        ...
        while self.__buffer:
            #Added this line to see responses
            if self.__buffer.startswith('URL:'):
                line, url = self.__buffer.split(': ', 1)
                url = url.strip()
                print "Our url: " + url
...
#        print(headerstr)
    ...
    def sendRequest(self, target='*', headers=None):
        """ will send a PIN code to the RTSP server. """
        self.sendMethod('REQUEST', target, headers)

    def sendAuth(self, target='*', headers=None):
        """the device will need to authenticate with the server and check they have a valid PIN number"""
        self.sendMethod('AUTH', target, headers)
...
class RTSPClientFactory(client.HTTPClientFactory):
    ...
    def __init__(self, url, filename, pin=None, timeout=0, agent=None, *args, **kwargs):
        ...
        if pin is None:
            print "No pin supplied so error"
        else:
            self.pin = pin
